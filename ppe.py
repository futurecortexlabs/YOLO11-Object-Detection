...
def run(image, confidence, required, sample, cache_dir):
    ...
    try:
        ...
        report = f'''<!doctype html><html lang="ja"><meta charset="utf-8"><title>PPE確認レポート</title><style>body{{font:15px sans-serif;max-width:1000px;margin:50px auto;color:#18382f}}table{{border-collapse:collapse;width:100%;margin:24px 0}}td,th{{padding:12px;border:1px solid #ddd;text-align:left}}small{{color:#666}}</style><h1>SAFE SIGHT / PPE確認レポート</h1><p>Future Cortex Labs · {timestamp}</p><p>{html.escape(message)}</p><p>確認対象: {html.escape(' / '.join(required))} / 信頼度しきい値: {confidence}</p>{table.to_html(index=False,escape=True)}<h2>モデル検出データ</h2>{raw.to_html(index=False,escape=True)}<p>座標は出力画像 {image.width} × {image.height}px 基準です。未検出は未着用を意味しません。現場担当者による確認を支援する参考情報です。</p><small>Model: {REPO} @ {REVISION if not os.environ.get('PPE_MODEL_PATH') else 'custom'} · 出典と利用条件はREADME参照</small></html>'''
